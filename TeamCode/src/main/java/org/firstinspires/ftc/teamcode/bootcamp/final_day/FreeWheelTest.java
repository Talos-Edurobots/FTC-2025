 ...
 *
 */
@TeleOp(name ="free wheel test")
//@Disabled
public class FreeWheelTest extends LinearOpMode {
    public DcMotor leftDrive, rightDrive;
    private static final double TICKS_PER_MM = 28 * 4 / (90 * Math.PI);
...
//            telemetry.update();
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("left pos encoder", leftDrive.getCurrentPosition());
            telemetry.addData("right pos encoder", rightDrive.getCurrentPosition());
            telemetry.addData("mm", leftDrive.getCurrentPosition()/TICKS_PER_MM);
            telemetry.update();
        }
//        if (toObservation) {
//            intake.setPosition(1);
